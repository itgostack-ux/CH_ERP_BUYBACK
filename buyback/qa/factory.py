...
# ── Payment methods (standard ERPNext Mode of Payment) ──────────
PAYMENT_METHODS = [
    {"name": "Cash", "type": "Cash"},
    {"name": "UPI", "type": "Phone"},
...
# ── CH Hierarchy (Category → SubCategory → Model) ───────────────
CH_CATEGORIES = [
    {"category_name": "QA Smartphones", "item_group": "Smartphones"},
    {"category_name": "QA Laptops", "item_group": "Laptops"},
    {"category_name": "QA Tablets", "item_group": "Tablets"},
]
CH_SUB_CATEGORIES = [
    {"category": "QA Smartphones", "sub_category_name": "QA Phone", "prefix": "QAP"},
    {"category": "QA Laptops", "sub_category_name": "QA Laptop", "prefix": "QAL"},
    {"category": "QA Tablets", "sub_category_name": "QA Tablet", "prefix": "QAT"},
]
# CH_MODELS created dynamically from ITEMS (one model per item)

# Map item_group → ch_category, ch_sub_category
_CH_MAP = {
    "Smartphones": {"ch_category": "QA Smartphones", "ch_sub_category": "QA Smartphones-QA Phone"},
    "Laptops": {"ch_category": "QA Laptops", "ch_sub_category": "QA Laptops-QA Laptop"},
    "Tablets": {"ch_category": "QA Tablets", "ch_sub_category": "QA Tablets-QA Tablet"},
}

ITEMS = [
    # Smartphones (10)  — item_code & item_name are auto-generated by ch_item_master
    {"model_name": "QA iPhone 15", "item_group": "Smartphones", "brand": "Apple", "market_price": 70000},
    {"model_name": "QA iPhone 14", "item_group": "Smartphones", "brand": "Apple", "market_price": 55000},
    {"model_name": "QA iPhone 13", "item_group": "Smartphones", "brand": "Apple", "market_price": 45000},
    {"model_name": "QA Samsung S24", "item_group": "Smartphones", "brand": "Samsung", "market_price": 65000},
    {"model_name": "QA Samsung S23", "item_group": "Smartphones", "brand": "Samsung", "market_price": 50000},
    {"model_name": "QA Samsung A34", "item_group": "Smartphones", "brand": "Samsung", "market_price": 22000},
    {"model_name": "QA Google Pixel 8", "item_group": "Smartphones", "brand": "Google", "market_price": 48000},
    {"model_name": "QA OnePlus 12", "item_group": "Smartphones", "brand": "OnePlus", "market_price": 55000},
    {"model_name": "QA Oppo Reno 12", "item_group": "Smartphones", "brand": "Oppo", "market_price": 30000},
    {"model_name": "QA Xiaomi 14", "item_group": "Smartphones", "brand": "Xiaomi", "market_price": 42000},
    # Laptops (5)
    {"model_name": "QA MacBook Pro M3", "item_group": "Laptops", "brand": "Apple", "market_price": 180000},
    {"model_name": "QA MacBook Air M2", "item_group": "Laptops", "brand": "Apple", "market_price": 100000},
    {"model_name": "QA Dell XPS 15", "item_group": "Laptops", "brand": "Dell", "market_price": 120000},
    {"model_name": "QA HP Spectre", "item_group": "Laptops", "brand": "HP", "market_price": 95000},
    {"model_name": "QA Lenovo ThinkPad", "item_group": "Laptops", "brand": "Lenovo", "market_price": 110000},
    # Tablets (5)
    {"model_name": "QA iPad Air", "item_group": "Tablets", "brand": "Apple", "market_price": 55000},
    {"model_name": "QA iPad Pro", "item_group": "Tablets", "brand": "Apple", "market_price": 90000},
    {"model_name": "QA Samsung Tab S9", "item_group": "Tablets", "brand": "Samsung", "market_price": 60000},
    {"model_name": "QA Lenovo Tab P12", "item_group": "Tablets", "brand": "Lenovo", "market_price": 30000},
    {"model_name": "QA Xiaomi Pad 6", "item_group": "Tablets", "brand": "Xiaomi", "market_price": 25000},
]
...
def seed_all(company: str | None = None):
    ...
    try:
        ...
        summary["manufacturers"] = _seed_manufacturers()
        summary["ch_hierarchy"] = _seed_ch_hierarchy()
...
def _seed_stores(company: str) -> int:
    ...
    for s in STORES:
        ...
        if not frappe.db.exists("Warehouse", wh_name):
            frappe.get_doc({
                "doctype": "Warehouse",
                "warehouse_name": s["store_name"],
                "company": company,
                "city": s["city"],
                "pin": s["pincode"],
                "ch_store_code": s["store_code"],
...
def _seed_payment_methods() -> int:
    """Ensure standard Mode of Payment entries exist (Cash, UPI, Bank Transfer)."""
    created = 0
    for pm in PAYMENT_METHODS:
        if not frappe.db.exists("Mode of Payment", pm["name"]):
            frappe.get_doc({"doctype": "Mode of Payment", "mode_of_payment": pm["name"], "type": pm["type"]}).insert(ignore_permissions=True)
            created += 1
    return created
...
def _seed_manufacturers() -> int:
    """Ensure manufacturer records exist for each unique brand in ITEMS."""
    created = 0
    seen = set()
    for item in ITEMS:
        brand = item["brand"]
        if brand in seen:
            continue
        seen.add(brand)
        if not frappe.db.exists("Manufacturer", brand):
            frappe.get_doc({"doctype": "Manufacturer", "short_name": brand}).insert(ignore_permissions=True)
            created += 1
    return created
...
def _seed_ch_hierarchy() -> int:
    ...
    # Ensure brand → manufacturer linkage
    seen_brands = set()
    for item in ITEMS:
    ...
    for cat in CH_CATEGORIES:
        if not frappe.db.exists("CH Category", cat["category_name"]):
            frappe.get_doc({
                "doctype": "CH Category",
    ...
    # Sub Categories (only include model_name in auto-generated item_name)
    _subcat_name_flags = {
        "include_manufacturer_in_name": 0,
        "include_brand_in_name": 0,
        "include_model_in_name": 1,
    }
    for sc in CH_SUB_CATEGORIES:
        expected_name = f"{sc['category']}-{sc['sub_category_name']}"
        if not frappe.db.exists("CH Sub Category", expected_name):
            frappe.get_doc({
            ...
            # Ensure name flags are correct on existing sub-categories
            frappe.db.set_value("CH Sub Category", expected_name, _subcat_name_flags)

    # Models (one per item)
    for item in ITEMS:
        model_name = item["model_name"]
        ch_map = _CH_MAP[item["item_group"]]
        if not frappe.db.exists("CH Model", {"model_name": model_name, "sub_category": ch_map["ch_sub_category"]}):
            frappe.get_doc({
                "doctype": "CH Model",
                "model_name": model_name,
                "sub_category": ch_map["ch_sub_category"],
...
def _seed_items() -> int:
    """Create QA items through the standard ch_item_master pipeline.

    Items are created with ch_model set, which triggers the ch_item_master
    before_insert hook to auto-generate item_code (from the sub-category
    prefix) and item_name (from the model name).  This follows the same
    flow used by real production data.

    After creation, serial_no_series is set using the auto-generated code.
    """
    hsn_map = {
    ...
    for item in ITEMS:
        ch_map = _CH_MAP[item["item_group"]]
        # Resolve CH Model name (primary key)
        ch_model = frappe.db.get_value(
            "CH Model",
            {"model_name": item["model_name"], "sub_category": ch_map["ch_sub_category"]},
            "name",
        )
        if not ch_model:
            frappe.log_error(f"_seed_items: CH Model not found for {item['model_name']}")
            continue

        # Skip if an item already exists for this model
        if frappe.db.exists("Item", {"ch_model": ch_model}):
            continue
        ...
        # Set serial_no_series using the auto-generated item_code
        frappe.db.set_value(
            "Item", doc.name, "serial_no_series", f"{doc.name}-.#####",
...
def _seed_price_masters() -> int:
    ...
    for item in ITEMS:
        ...
        if not ch_model:
            continue
        actual_code = frappe.db.get_value("Item", {"ch_model": ch_model}, "name")
        if not actual_code:
            continue
        if frappe.db.exists("Buyback Price Master", {"item_code": actual_code}):
            continue
        mp = item["market_price"]
        item_name = frappe.db.get_value("Item", actual_code, "item_name")
        doc = frappe.get_doc({
...
def _cleanup_transactions():
    """Delete ALL QA transactions in dependency order.

    Uses direct SQL for speed, handles:
    - Cancel submitted JE/SE/Orders first
    - Delete child tables before parent tables
    - Clean linked GL/SLE, Comments, Versions, Serial Nos
    """
    # ── Cancel submitted accounting entries ──────────────────────
    # Journal Entries linked to buyback
    for name in frappe.db.sql_list(
        "SELECT name FROM `tabJournal Entry` WHERE user_remark LIKE '%%Buyback%%' AND docstatus=1"
    ):
        try:
            ...
            frappe.db.sql("UPDATE `tabJournal Entry` SET docstatus=2 WHERE name=%s", name)

    # Stock Entries linked to buyback
    for name in frappe.db.sql_list(
        "SELECT name FROM `tabStock Entry` WHERE remarks LIKE '%%Buyback%%' AND docstatus=1"
    ):
        try:
            ...
            frappe.db.sql("UPDATE `tabStock Entry` SET docstatus=2 WHERE name=%s", name)

    # ── Cancel submitted buyback docs ────────────────────────────
    for name in frappe.db.sql_list(
        "SELECT name FROM `tabBuyback Exchange Order` WHERE docstatus=1"
    ):
        try:
            ...
            frappe.db.sql("UPDATE `tabBuyback Exchange Order` SET docstatus=2 WHERE name=%s", name)

    for name in frappe.db.sql_list(
        "SELECT name FROM `tabBuyback Order` WHERE docstatus=1"
    ):
        try:
            ...
            frappe.db.sql("UPDATE `tabBuyback Order` SET docstatus=2 WHERE name=%s", name)

    frappe.db.commit()

    # ── SQL bulk delete (much faster than doc-by-doc) ────────────
    # Child tables first
    for child_table in (
        "tabBuyback Inspection Comparison",
        "tabBuyback Inspection Result",
        "tabBuyback Assessment Response",
        "tabBuyback Order Payment",
    ...
    # Parent tables
    for parent_table in (
        "tabBuyback Exchange Order",
        "tabBuyback Order",
        "tabBuyback Inspection",
        "tabBuyback Assessment",
        "tabBuyback Audit Log",
    ...
    # JE + JE accounts
    frappe.db.sql(
        "DELETE FROM `tabJournal Entry Account` WHERE parent IN "
        "(SELECT name FROM `tabJournal Entry` WHERE user_remark LIKE '%%Buyback%%')"
    )
    frappe.db.sql("DELETE FROM `tabJournal Entry` WHERE user_remark LIKE '%%Buyback%%'")

    # SE + SE details
    frappe.db.sql(
        "DELETE FROM `tabStock Entry Detail` WHERE parent IN "
        "(SELECT name FROM `tabStock Entry` WHERE remarks LIKE '%%Buyback%%')"
    )
    frappe.db.sql("DELETE FROM `tabStock Entry` WHERE remarks LIKE '%%Buyback%%'")

    # GL Entries from JEs (they reference cancelled/deleted JEs)
    frappe.db.sql(
        "DELETE FROM `tabGL Entry` WHERE voucher_type='Journal Entry' "
        "AND voucher_no NOT IN (SELECT name FROM `tabJournal Entry`)"
    )

    # Stock Ledger Entries from SEs
    frappe.db.sql(
        "DELETE FROM `tabStock Ledger Entry` WHERE voucher_type='Stock Entry' "
        "AND voucher_no NOT IN (SELECT name FROM `tabStock Entry`)"
    )

    # QA Serial Nos (item_codes now start with QAP/QAL/QAT prefixes)
    frappe.db.sql(
        "DELETE FROM `tabSerial No` WHERE item_code IN "
        "(SELECT name FROM `tabItem` WHERE ch_model IN "
        "(SELECT name FROM `tabCH Model` WHERE model_name LIKE 'QA %%'))"
    )
    ...
    # Comments & Versions for buyback doctypes
    frappe.db.sql("DELETE FROM `tabComment` WHERE reference_doctype LIKE '%%Buyback%%'")
    frappe.db.sql("DELETE FROM `tabVersion` WHERE ref_doctype LIKE '%%Buyback%%'")

    # Reset naming series
    for series in ("BBO-.YYYY.-.#####", "BBQ-.YYYY.-.#####", "BBI-.YYYY.-.#####", "BBA-.YYYY.-.#####"):
...
def _cleanup_masters():
    ...
    # Price masters (item codes are auto-generated, look up via QA CH Models)
    qa_item_codes = frappe.get_all(
        "Item",
        filters={"ch_model": ["in",
            frappe.get_all("CH Model", {"model_name": ["like", "QA %"]}, pluck="name") or ["__none__"]
        ]},
        pluck="name",
    )
    for name in frappe.get_all("Buyback Price Master", {"item_code": ["in", qa_item_codes or ["__none__"]]}, pluck="name"):
        frappe.delete_doc("Buyback Price Master", name, force=True, ignore_permissions=True)

    # Items (auto-generated codes linked to QA CH Models)
    for item_code in qa_item_codes:
        frappe.delete_doc("Item", item_code, force=True, ignore_permissions=True)

    # CH Models (QA-prefixed)
    for item in ITEMS:
        ch_map = _CH_MAP.get(item["item_group"], {})
        if ch_map:
            model = frappe.db.get_value(
                "CH Model",
                {"model_name": item.get("model_name", ""), "sub_category": ch_map.get("ch_sub_category", "")},
                "name",
            )
            if model:
    ...
    for sc in CH_SUB_CATEGORIES:
        expected_name = f"{sc['category']}-{sc['sub_category_name']}"
        if frappe.db.exists("CH Sub Category", expected_name):
            frappe.delete_doc("CH Sub Category", expected_name, force=True, ignore_permissions=True)

    # CH Categories
    for cat in CH_CATEGORIES:
        if frappe.db.exists("CH Category", cat["category_name"]):
            frappe.delete_doc("CH Category", cat["category_name"], force=True, ignore_permissions=True)
    ...
    # Payment methods — only delete non-standard ones we created
    for pm in PAYMENT_METHODS:
        if pm["name"] not in ("Cash",):  # Don't delete built-in Cash
            if frappe.db.exists("Mode of Payment", pm["name"]):
                frappe.delete_doc("Mode of Payment", pm["name"], force=True, ignore_permissions=True)
...
def get_item(model_fragment: str) -> str:
    """Return the auto-generated Item code for a QA item by model name fragment.

    Looks up the CH Model whose ``model_name`` matches ``QA %<fragment>%``
    and returns the linked Item's primary key (the auto-generated item_code).

    Examples::

        get_item("iPhone 15")    # → "QAP000001"
        get_item("Samsung S24")  # → "QAP000004"
        get_item("MacBook Pro")  # → "QAL000001"
    """
    ch_model = frappe.db.get_value(
        "CH Model",
        {"model_name": ["like", f"QA %{model_fragment}%"]},
        "name",
    )
    if not ch_model:
        frappe.throw(f"QA get_item: No CH Model matching 'QA %{model_fragment}%'")
    item_code = frappe.db.get_value("Item", {"ch_model": ch_model}, "name")
    if not item_code:
        frappe.throw(f"QA get_item: No Item found with ch_model={ch_model}")
    return item_code


def get_store(code: str = "QA-ANN") -> str:
    """Return Warehouse name by ch_store_code."""
    return frappe.db.get_value("Warehouse", {"ch_store_code": code}, "name")
...
def get_payment_method(method_type: str = "Cash") -> str:
    """Return Mode of Payment name by type.

    Maps legacy method_type values to Mode of Payment names:
      Cash  → 'Cash'
      UPI   → 'UPI'
      Bank  → 'Bank Transfer'
    """
    type_to_name = {"Cash": "Cash", "UPI": "UPI", "Bank": "Bank Transfer"}
    name = type_to_name.get(method_type, method_type)
    if frappe.db.exists("Mode of Payment", name):
        return name
    # Fallback: search by type field
    return frappe.db.get_value("Mode of Payment", {"type": method_type}, "name") or "Cash"
